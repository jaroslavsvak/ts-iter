...
class IterableWrapper {
     ...
     */
    toReadonlyMap(keyMapper) {
        return this.toMap(keyMapper);
    }
    /**
     ...
     * @returns `IterableWrapper<T>` over the full tree-like structure.
     */
    flattenAndMap(nestedAccessor, mapper) {
        const inner = () => {
            nestedAccessor = nestedAccessor || (x => Array.isArray(x) ? x : undefined);
            function* walk(iterator, level) {
                for (const item of iterator) {
